"""Configuration generator for SeapoPym pteropods from Bednarsek equations, and considering Berverton Holt density dependance."""
...
def pteropod_bed_bh_functional_group_unit_generator(
    functional_group: PteropodBedBHFunctionalGroup,
) -> FunctionalGroupUnit:
    ...
    return FunctionalGroupUnit(
        ...
        functional_type=FunctionalTypeParameter(
            ...
            density_dependance_parameter_a=functional_group.density_dependance_parameter_a,
            density_dependance_parameter_b=functional_group.density_dependance_parameter_b,
        ),
    )


@dataclass
class PteropodsBedBHConfigurationGenerator:
    """Generate the configuration used to create a Pteropod model in SeapoPym based on Bednarsek equations, and considering Berverton Holt density dependance.

    Based on `ConfigurationGeneratorProtocol`.
    """

    model_class: type[AcidityBedBHSurvivalModel] = AcidityBedBHSurvivalModel

    def generate(
        self,
        functional_group_parameters: Sequence[PteropodBedBHFunctionalGroup],
        forcing_parameters: ForcingParameter,
        kernel: KernelParameter | None = None,
    ) -> AcidityBedBHConfiguration:
        """Generate a AcidityBedBHConfiguration with the given functional groups and parameters."""
        functional_groups_converted = [
            pteropod_bed_bh_functional_group_unit_generator(fg) for fg in functional_group_parameters
        ]
        return AcidityBedBHConfiguration(
